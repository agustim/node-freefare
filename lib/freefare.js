...
class MifareDesfireTag extends Tag {
	...
	* @return {Promise} A promise to the end of the action.
	*/
	authenticateAES(KeyNum, key) {
		return new Promise((resolve, reject) => {
			this[cppObj].mifareDesfire_authenticate_aes(KeyNum, key, (error) => {
				if(error) {
					switch (error) {
						default:
						reject(new Error('Unknown error (' + error + ')'));
	...
	*/
	...
	/**
	* Read all the given file
	* @param {Number} file The file ID
	* @return {Promise<Buffer>} A promise to the read data
	*/
	readFile(file) {
		return new Promise((resolve, reject) => {
			this[cppObj].mifareDesfire_readFile(file, (error, result) => {
				if(error) {
					switch (error) {
						default:
						reject(new Error('Unknown error (' + error + ')'));
